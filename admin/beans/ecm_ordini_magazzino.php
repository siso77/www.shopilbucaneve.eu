...
class ecm_ordini_magazzino extends BeanBase
{
	...
	function dbGetAllByIdOrdine($db=null, $idOrdine, $BeanMagazzino = null, $BeanContent = null)
	{
		if (!$this->_is_connection($db))
			return false;
		...
		while($row=$result->fetchRow(DB_FETCHMODE_ASSOC))
		{
			...
			{
				$data = $BeanMagazzino->dbSearch($db, ' AND magazzino.id = '.$row['id_magazzino']);
				$row['magazzino'] = $data[0];
			} 
			if(!empty($BeanContent))
			{
				$data = $BeanContent->dbSearch($db, ' AND content.id = '.$row['id_content']);
				$row['content'] = $data[0];
			} 
			$values[]=$row;
