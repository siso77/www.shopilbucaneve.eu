...
//					$this->_redirect('?act=CaricaMagazzinoStepUno&confirm_insert=1');
				
				$this->_redirect('?act=CaricaMagazzino&id_content='.$idContent);				
	...
	function convertTplData($data)
	{
		...
		if(key_exists('is_invisible', $data))
				$ret['is_invisible'] = 1;
		else
			$ret['is_invisible'] = 0;
		...
		if(empty($data['price_it']))
			...
			$ret['price_it'] = $this->FormatEuro($data['price_it']);
		
		if(empty($data['price_discounted_it']))
			$ret['price_discounted_it'] = $this->FormatEuro(0);
		else
			$ret['price_discounted_it'] = $this->FormatEuro($data['price_discounted_it']);
