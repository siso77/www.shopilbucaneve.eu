...
class users_type extends BeanBase
{
	var $id;
	var $name;
	var $description;
	...
	function users_type($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = 'users_type';
	...
	function dbGetOneByParams($db=null, $params=null)
	{
		if (!$this->_is_connection($db))
			return false;

		$query="SELECT * FROM ".$this->table_name." WHERE params LIKE '%". $params . "%'";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function dbDelete($db=null, $id=null, $is_logical = false)
	{
		$query = "DELETE FROM ".$this->table_name." WHERE id = ".$id[0];
		$db->query($query);
		//		Loggo la query sql
		$this->BeanLog("PEAR_DB", $db);
		//		Loggo la query sql		
	...
	function getPermessi(){return $this->permessi;}

	function setPermessi($value= null)
	{
		$this->permessi = $value;
	}
}
