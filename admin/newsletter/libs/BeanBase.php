...
class BeanBase
{
	...
	function BeanBase()
	{
		if(IF_LOG)
		{
			...
			$_SESSION['SYSTEM_LOG']['MESSAGE_LOG'] = " Utente di dominio: [ ".$_SESSION['SYSTEM_LOG']['USER']." ] " .
					"REMOTE IP: [ ".$_SESSION['SYSTEM_LOG']['REMOTE_PORT']." ] " .
					"REMOTE PORT: [ ".$_SESSION['SYSTEM_LOG']['REMOTE_IP']." ] " .
					"REMOTE MAC Address: [ ".$_SESSION['SYSTEM_LOG']['REMOTE_MAC_ADDRESS']." ] " .
	...
	function BeanPearDbLog($db)
	{
		$msg = " E' stata eseguita la query: [ ".$db->last_query." ] dall'utente di dominio [ ".$_SESSION['UserData']['username']." ".$_SESSION['USER_FULL_NAME']." ]";
		MyLog::sql_log($msg);
	} 
