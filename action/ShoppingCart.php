		...
		*/
			...
			foreach ($_SESSION[session_id()]['basket'] as $key => $value)
			{
				if($key != 'n_carrelli' && $key != 'perc_occupazione')
				{
					if($value['giacenza']['id'] == $_REQUEST['id_giacenza'])
					{
						...
						$BeanMagazzino->setDisponibilita(($BeanMagazzino->getDisponibilita()+($value['basket_qty']['sel_quantita'])*$content['qta_min_ord']));
		...
		if(!empty($_REQUEST['quantita']))
		{
			...
			if(!empty($_REQUEST['is_ajax']))
			{
				...
				$Bean = new content($this->conn, $_REQUEST['id_content']);
				$content = $Bean->vars();
				
/***
 * Nuova Logica per gestire bene le quantita a carrello
*/
if($BeanGiacenze->getDisponibilita() < $_REQUEST['quantita'])
{
	echo "<script>jQuery.fancybox('Attenzione, la quantit&aacute; richiesta non &eacute; pi&uacute; disponibile!');</script>";
	exit();
}
/***
 * Nuova Logica per gestire bene le quantita a carrello
*/
				...
				$BeanMagazzino->setDisponibilita(($BeanMagazzino->getDisponibilita())-($_REQUEST['quantita']));
				$BeanMagazzino->dbStore($this->conn);
				...
				foreach ($_SESSION[session_id()]['basket'] as $k => $value)
				{
					if($value['giacenza']['id'] == $giacenza['id'])
					{
						...
						$_SESSION[session_id()]['basket'][$k]['price_it_qty'] = (str_replace(',', '.', $giacenza[$this->key_prezzo]) * $_REQUEST['quantita'] );
				...
				if(!$in_session)
				{
					...
					$_SESSION[session_id()]['basket'][$index]['price_it_qty'] = (str_replace(',', '.', $giacenza[$this->key_prezzo]) * $_REQUEST['quantita'] );
				...
				*/
				if(is_array($_REQUEST['quantita']))
				{
					foreach ($_REQUEST['quantita'] as $key => $quantita)
					{
						...
						$Bean = new content($this->conn, $_REQUEST['id_contenuto'][$key]);
						$content = $Bean->vars();
						
						
						$this->key_prezzo = $this->getPriceByQty($giacenza, $quantita);
						$BeanMagazzino = new giacenze($this->conn, $_REQUEST['id_giacenza'][$key]);
						$BeanMagazzino->setDisponibilita( ($BeanMagazzino->getDisponibilita())-($quantita) );
						$BeanMagazzino->dbStore($this->conn);
						
						$_SESSION[session_id()]['basket'][$key+1]['basket_qty']['sel_quantita'] = $quantita;
						$_SESSION[session_id()]['basket'][$key+1]['basket_qty']['quantita'] = $quantita;
						$_SESSION[session_id()]['basket'][$key+1]['volume'] = $giacenza['volume_singolo'];
						$_SESSION[session_id()]['basket'][$key+1]['price_it_qty'] = ( str_replace(',', '.', $giacenza[$this->key_prezzo]) * $quantita );
				...
				else
				{
					if(!empty($_REQUEST['id_giacenza']))
					{
						...
						$_SESSION[session_id()]['basket'][$key]['basket_qty']['quantita'] = $_REQUEST['quantita'];
						$_SESSION[session_id()]['basket'][$key]['volume'] = $giacenza['volume_singolo'];
						$_SESSION[session_id()]['basket'][$key]['price_it_qty'] = ( str_replace(',', '.', $giacenza[$this->key_prezzo]) * $_REQUEST['quantita'] );
