<?php
class ImportGasa extends DBSmartyMailAction
{
	function ImportGasa()
	{
		...
		{
			$pop3=IMPORT_GIACENZE_GASA_EMAIL_ADMIN_HOST;
			$username=IMPORT_GIACENZE_GASA_EMAIL_ADMIN_USERNAME;
			$password=IMPORT_GIACENZE_GASA_EMAIL_ADMIN_PASSWORD;
			...
			$user=$username;
			$pass=$password;
			$savedirpath = APP_ROOT.'/admin/upload_fornitori/gasa/';
...
// 					"Cc" 			=> "",
// 					"Bcc" 		=> "",
// 					"Subject" 	=> "Importazione Listino Gasa per Gardesana",
// 					"Date"		=> date("r")
// 			);
// 			$this->setHeaders($hdrs);
// 			$this->setHtmlText('IMPORTAZIONE MAGAZZINO GASA AVVENUTA CON SUCCESSO!');
// 			$this->mail_factory();
// 			$is_send = $this->sendMail('siso77@gmail.com');
	...
	function getdata($host,$login,$password,$savedirpath,$delete_emails=false, $read_type="UNSEEN") 
	{
		...
		$hdrs = array("From" 		=> EMAIL_ADMIN_FROM,
				...
				"Cc" 			=> "",
				"Bcc" 		=> "",
				"Subject" 	=> "FILE EXCEL MAGAZZINO FORNITORE 2",
				"Date"		=> date("r")
		);
		$this->setHeaders($hdrs);
