...
// 				$confirm = true;
		...
		if(!empty($confirm))
		{
			if(!empty($_SESSION[session_id()]['ecm_id_ordine']))
			{
				...
				if(!empty($_SESSION['LoggedUser']['customer_data']['customer_code']))
					$BeanCustomer = new customer($this->conn, $_SESSION['LoggedUser']['customer_data']['id']);
				else
				...
				if(!empty($_SESSION[session_id()]['basket']))
				{
					...
					foreach ($tmp as $key => $value)
					{
						...
						$str .= $this->tEngine->getBlankSpaceForFlorSysOrder(($value['basket_qty']['sel_quantita']*$content['qta_min_ord']), 10);
						if ($value['giacenza']['stato'] == 'N')
						...
						$str .= $this->tEngine->getBlankSpaceForFlorSysOrder($value['selected_color'], 80);
						/* AGGIUNTO COLORE */
				...
				if(!empty($products_fornitori))
				{
					$values=array();
					$query="SELECT * FROM ecm_ordini_seq";
					$result=$this->conn->query($query);
					while($row=$result->fetchRow(DB_FETCHMODE_ASSOC))
						$values=$row;
					$result->free();
					$query="UPDATE ecm_ordini_seq SET id = ".($values['id']+1);
					$result=$this->conn->query($query);
					
					$fp = fopen(APP_ROOT.'/FlorSysIntegration/Out/0000088V.'.$this->tEngine->getCutomFormatCode($values['id']+1, 3), 'w+');
					fwrite($fp, "*MAG#1\r\n");
					if(!empty($products_fornitori))
						$this->generateEtiflorFileOrder($products_fornitori, $fp, new giacenze_fornitori());
					fclose($fp);
				}

				if(!empty($products_fornitori_de))
				{
					$values=array();
					$query="SELECT * FROM ecm_ordini_seq";
					$result=$this->conn->query($query);
					...
					fwrite($fp, "*MAG#1\r\n");
					if(!empty($products_fornitori_de))
						$this->generateEtiflorFileOrder($products_fornitori_de, $fp, new giacenze_forn_gasa());
					fclose($fp);
				}
...
// 					$this->tEngine->getRicarico($prezzo_acquisto, $this->commissioneScatolaDenDekker[0]['name']);
	...
	function SendEmail($BeanEcmOrdini, $userAnag, $products, $products_fornitori, $products_fornitori_de)
	{
		...
		if(!empty($_SESSION['LoggedUser']['customer_data']['id_agent']) || $_SESSION['LoggedUser']['username'] == 'davide')
		{
			$BeanCustomer = new customer($this->conn, $_SESSION['LoggedUser']['customer_data']['id']);
			$userAnag['name'] = $BeanCustomer->getNome();
			$userAnag['surname'] = $BeanCustomer->getCognome();
			$userAnag['address'] = $BeanCustomer->getIndirizzo();
			$userAnag['cap'] = $BeanCustomer->getCap();
			$userAnag['citta'] = $BeanCustomer->getCitta();
			$userAnag['provincia'] = $BeanCustomer->getProvincia();
			$userAnag['email'] = $BeanCustomer->getEmail();
		}
...
// 						if($_SESSION['LoggedUser']['scorporo_iva'])
// 						{
// 							$codice_iva = '1.'.$BeanGiacenze['cod_iva'];
// 							$prezzo_iva = (str_replace(',', '.', $product['importo']) / $codice_iva);
// 							$imponibile = (str_replace(',', '.', $product['importo']) / $codice_iva);
// 							$prezzo_iva = str_replace(',', '.', $product['importo'])-$prezzo_iva;
...
// 						else
// 						{
if ($BeanGiacenze['stato'] == 'N'){
		$BeanGiacenze[Currency::getPriceByQty($BeanGiacenze, $content['qta_min_ord'])] = $BeanGiacenze['prezzo_7'];
		$product['importo'] = $BeanGiacenze['prezzo_7'] * $product['quantita'];
	}
							$prezzo_iva = round( (str_replace(',', '.', $product['importo']) * $BeanGiacenze['cod_iva']) / 100,2);
							$imponibile = str_replace(',', '.', $product['importo']);
							$total = $total + str_replace(',', '.', $product['importo'])+$prezzo_iva;
							$tot_prezzo_iva = $tot_prezzo_iva + $prezzo_iva;
// 						}

// 						$speseSpedizioneIva = round( $speseSpedizione[0]['name'] * ('0.'.$BeanGiacenze['cod_iva']) , 2);
// 						$speseSpedizione[0]['name'] = $speseSpedizione[0]['name']+$speseSpedizioneIva;
						...
						if($key == 0)
						{
							$html .='
								<tr style="color:#000000;font-size:16px;">
									...
									<td>Nome</td>
									<td>Colore</td>
									<td align="center">Quantit&aacute; Imballo</td>
									<td align="center">Imballi Acquistati</td>
									<td align="center">Prezzo</td>';
									$html .='<td align="center">IVA</td>';
									$html .='<!--<td align="center">Importo</td>-->
									<td style="min-width:200px">Note</td>
									<!--<td align="center">Indispensabile x Spedizione</td>-->
								</tr>';
						}
						$image = null;
						$image = $this->tEngine->getImageFromVbn($product['vbn']);
						if(empty($image)){
							$obj_image = $this->tEngine->dbGetImageFromBarCode($BeanGiacenze['bar_code']);
						...
						if(!empty($image))
							...
							$image = '<img src="'.$_tplvar['WWW_ROOT'].$_tplvar['IMG_DIR'].'/image_large.gif" width="90">';
						
						$html .='
							<tr style="color:#000000;font-size:16px;">
								<td align="center">'.$image.'</td>
								<td align="center">'.$BeanGiacenze['bar_code'].'</td>
								<td nowrap="nowrap">'.$BeanGiacenze['descrizione'].'</td>
								<td nowrap="nowrap">'.$product['colore'].'</td>
								<td align="center">'.$product['quantita'].'</td>
								<td align="center">'.$content['qta_min_ord'].'</td>
								<td align="center">'.Currency::FormatEuro($BeanGiacenze[Currency::getPriceByQty($BeanGiacenze, $content['qta_min_ord'])]).'</td>';
								$html .='<td align="center">'.$BeanGiacenze['cod_iva'].'</td>';
								$html .='<!--<td align="center">'.Currency::FormatEuro($product['importo']).'</td>-->
								<td>'.$product['nota'].'</td>';
						if(!empty($product['indispensabile']))									
					...
					if(!empty($imponibile))
					{
						...
						{
							$iva_spese = round( (str_replace(',', '.', $speseSpedizione[0]['name']) * $BeanGiacenze['cod_iva']) / 100,2);
							$html .= '<td align="right">Spese Spedizione</td>';
							$html .= '<td>'.$this->tEngine->getFormatPrice($speseSpedizione[0]['name']+$iva_spese).'&euro; </td>';
							$total = str_replace(',', '.', $total)+str_replace(',', '.', $speseSpedizione[0]['name'])+$iva_spese;
						}
						else
						{
						...
						$html .= '<td>'.Currency::FormatEuro($tot_prezzo_iva).'</td>';
						$html .= '</tr>';

						$html .= '<tr style="color:#000000;font-size:16px;">';
						$html .= '<td align="right">Totale</td>';
						$html .= '<td>'.Currency::FormatEuro( $total ).'</td>';
						$html .= '</tr>';
						$html .= '</table>';
						$html .= '</td>';
						$html .= '</tr>';
...
// 					}
		...
		if(!empty($_SESSION['LoggedUser']['customer_data']))
			$BeanCustomer = new customer($this->conn, $_SESSION['LoggedUser']['customer_data']['id']);
		else
			$BeanCustomer = new customer($this->conn, $_SESSION['LoggedUser']['id_customer']);
_dump($userAnag);
echo $html.$html_n_carrelli.$html_footer;
exit();
