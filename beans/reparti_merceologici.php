...
class reparti_merceologici extends BeanBase
{
	var $id;
	var $reparto;

	function reparti_merceologici($conn=null, $id=null)
	{
		parent::BeanBase();

		$this->table_name = "reparti_merceologici";
	...
	function dbGetByReparto($db=null, $reparto = null)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=false;
		$query="SELECT * FROM ".$this->table_name." WHERE reparto = '".$reparto."'";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
...
$this->setID($id);
	...
	function setId($value= null)
	{
		...
		function getReparto(){return $this->reparto;}

	function setReparto($value= null)
	{
				if(strlen($value) > 255)
			$value = substr($value, 0, 255);
				
								
		$this->reparto = (string)$value;
	}
