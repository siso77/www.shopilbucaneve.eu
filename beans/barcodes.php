...
class barcodes extends BeanBase
{
	var $id;
	var $codice_articolo;
	var $bar_code;

	function barcodes($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "barcodess";
	...
	function dbGetAllByIdCategory($db=null, $id)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE id_category = ".$id;
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function dbSearch($db=null, $search=null)
	{
		...
		$query = "SELECT * FROM barcodes WHERE ".$search;
	...
	function setCodice_articolo($value= null)
	{
		...
		$this->codice_articolo = (int)$value;
	}
