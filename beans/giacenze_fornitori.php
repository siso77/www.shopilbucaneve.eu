...
$this->setID($id);
	...
	function dbGetNomeFornitore($db=null, $search=null)
	{
		if (!$this->_is_connection($db))
			return false;
		...
		$query="SELECT fornitore FROM ".$this->table_name." WHERE is_active = 1 ".$search." GROUP BY fornitore ";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function dbGetGm($db=null, $search=null)
	{
		...
		$query="SELECT categoria as stato FROM ".$this->table_name." WHERE is_active = 1 ".$search." GROUP BY categoria ";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function getReferenza(){return $this->referenza;}
	
	function setReferenza($value= null)
	{
		if(strlen($value) > 255)
			$value = substr($value, 0, 255);
	
		$this->referenza = (string)$value;
	}
}
?>