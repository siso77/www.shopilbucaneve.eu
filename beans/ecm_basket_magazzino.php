...
class ecm_basket_magazzino extends BeanBase
{
	var $id;
	var $id_basket;
	var $id_magazzino;
	...
	function ecm_basket_magazzino($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "ecm_basket_magazzino";
	...
	function dbGetOneByBasketAndMagazzino($db=null, $id_basket=null, $id_magazzino = null)
	{
		if (!$this->_is_connection($db))
			return false;

		$query="SELECT * FROM ".$this->table_name." WHERE id_basket=". $id_basket." AND id_magazzino = ".$id_magazzino;
	...
	function dbGetAllByIdBasket($db=null, $idBasket)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE id_basket = ".$idBasket. ' AND is_active = 1';
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function getId_basket(){return $this->id_basket;}

	function setId_basket($value= null)
	{
				if(strlen($value) > 11)
			$value = substr($value, 0, 11);
				
								
		$this->id_basket = (int)$value;
	}
	...
	function getColore(){return $this->colore;}

	function setColore($value= null)
	{
				if(strlen($value) > 11)
			$value = substr($value, 0, 11);
